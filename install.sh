...
if [ ! -d "$SYSTEM_DIR/usr/local/bin/aircoins-api" ]; then
    ...
    echo ""
    echo -e "    git clone https://github.com/iamxfullz/AirCoins.git /opt/aircoins"
...
if [ -f "$SCRIPT_DIR/.env" ]; then
    ...
    if [ "$SCRIPT_DIR" != "/opt/aircoins" ]; then
...
elif [ -f "/opt/aircoins/.env" ]; then
    chown root:root /opt/aircoins/.env
    chmod 600 /opt/aircoins/.env
    echo "  ✓ /opt/aircoins/.env preserved (license credentials)"
elif [ -f "$SCRIPT_DIR/.env.example" ]; then
    mkdir -p /opt/aircoins
    cp "$SCRIPT_DIR/.env.example" /opt/aircoins/.env
    chown root:root /opt/aircoins/.env
    chmod 600 /opt/aircoins/.env
    echo "  ✓ .env.example → /opt/aircoins/.env (license credentials)"
elif [ -f "/opt/aircoins/.env.example" ]; then
    cp "/opt/aircoins/.env.example" /opt/aircoins/.env
    chown root:root /opt/aircoins/.env
    chmod 600 /opt/aircoins/.env
    echo "  ✓ /opt/aircoins/.env.example → /opt/aircoins/.env (license credentials)"
else
    echo -e "  ${YELLOW}⚠ No .env file found. License system requires SUPABASE_URL and SUPABASE_ANON_KEY.${NC}"
